...
export function CarCardClassic({
    title,
    slug,
    ...
    return (
        <div className="group bg-white rounded-xl overflow-hidden border border-[#e0e3e6]/60 hover:shadow-xl hover:-translate-y-1 transition-all duration-400 flex flex-col h-full">
            <div className="relative aspect-[16/9] w-full overflow-hidden bg-[#f2f4f7]">
                <Image src={image || "/car-placeholder.png"} alt={title} fill className="object-contain p-4 group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
                {category && (
            ...
            <div className="p-5 md:p-6 flex flex-col flex-1">
                <div className="mb-4">
                    <h3 className="font-headline text-lg md:text-xl font-bold text-[#191c1e] leading-tight">{title}</h3>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-4 text-[13px] text-[#444651]">
                    ...
                    <div className="flex items-center gap-2"><span className="material-symbols-outlined text-base text-[var(--color-primary)]">ac_unit</span>Climatisation</div>
                    {caution && <div className="flex items-center gap-2"><span className="material-symbols-outlined text-base text-[var(--color-primary)]">shield</span>{caution}</div>}
                </div>
                <div className="border-t border-[#e0e3e6]/60 pt-4 mt-auto">
                    <div className="flex items-end justify-between mb-4">
                        <div>
                            <p className="text-[11px] text-[#444651] uppercase tracking-wide font-medium">Prix pour 3 jours</p>
                    ...
                    <Link href={`/rental/${citySlug}/${slug}`} className="block w-full py-3.5 bg-[var(--color-primary)] text-white font-bold rounded-lg text-center text-sm uppercase tracking-wider hover:opacity-90 transition-opacity duration-300">
                        Voir l&apos;offre
                    </Link>
                </div>
            </div>
